...
@admin_only
def home(request):
    breakfast=Meal.objects.filter(name='breakfast')[0].fooditem_set.all()[:5]
    lunch=Meal.objects.filter(name='lunch')[0].fooditem_set.all()[:5]
    dinner=Meal.objects.filter(name='dinner')[0].fooditem_set.all()[:5]
    snacks=Meal.objects.filter(name='snacks')[0].fooditem_set.all()[:5]
...
def fooditem(request):
    breakfast=Meal.objects.filter(name='breakfast')[0].fooditem_set.all()
    bcnt=breakfast.count()
    lunch=Meal.objects.filter(name='lunch')[0].fooditem_set.all()
    lcnt=lunch.count()
    dinner=Meal.objects.filter(name='dinner')[0].fooditem_set.all()
    dcnt=dinner.count()
    snacks=Meal.objects.filter(name='snacks')[0].fooditem_set.all()
    scnt=snacks.count()
    context={'breakfast':breakfast,
              'bcnt':bcnt,
              'lcnt':lcnt,
...
def registerPage(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            Client.objects.create(user=user, name=username,email=email)
